...
func _parse_begin(object):
	var trigger := object as MEFEventTrigger
	if not trigger:
		return
	if trigger.preset:
		_draw_apply_button(trigger)

func _draw_apply_button(trigger: MEFEventTrigger) -> void:
	var button := Button.new()
	button.text = "Apply MEF Preset"
	button.pressed.connect(func():
		pending_trigger = trigger
		_show_confirmation(trigger)
	)
...
func _show_confirmation(trigger: MEFEventTrigger) -> void:
	...
	if _has_children(trigger):
		confirm_dialog.dialog_text = "This will replace existing Conditions and Effects.\nContinue?"
	else:
		confirm_dialog.dialog_text = "Apply preset to this trigger?"
...
func _apply_preset_with_undo(trigger: MEFEventTrigger) -> void:
	...
	for child in trigger.get_children():
		if child is MEFCondition and child.scene_file_path != "":
			old_condition_paths.append(child.scene_file_path)
		elif child is MEFEffect and child.scene_file_path != "":
			old_effect_paths.append(child.scene_file_path)
	undo_redo.create_action("Apply MEF Preset")
	undo_redo.add_do_method(trigger, "_apply_preset_internal", trigger.preset)
	undo_redo.add_undo_method(trigger, "_restore_from_scene_paths", old_condition_paths, old_effect_paths)
	undo_redo.commit_action()

func _has_children(trigger: MEFEventTrigger) -> bool:
	for child in trigger.get_children():
